...
use banderwagon::scalar_multi::MSMExtendPrecompWnaf;
use sysinfo::System;
...
fn benchmark_std_2dasm_precompute_mul(c: &mut Criterion) {
    let mut system = System::new_all();
    system.refresh_all();
    for (i, cpu) in system.cpus().iter().enumerate() {
        println!("CPU {} frequency: {} MHz", i, cpu.frequency());
    ...
    for &window in &windows {
        let precompute = MSMExtendPrecompWnaf::new(&basic_crs, window);
        let mem_byte_size= precompute.tables.len()*precompute.tables[0].len()*4*32;
        println!("precompute_byte_size: {:?}", mem_byte_size);
...
criterion_group!(benches, benchmark_std_2dasm_precompute_mul);//benchmark_gotti_precompute_mul benchmark_gotti_precompute_mul
